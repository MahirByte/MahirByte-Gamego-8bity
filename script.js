...
class Emulator {
  ...
  render() {
    ...
    if (this.state === "running" && this.currentRom) {
      ...
      if (romState?.nesMeta) {
        drawText("NES EMU PREVIEW MODE", 40, 206, COLOR_PALETTE.cyan);
      }
      return;
    }
  }
...
function decodeINES(bytes) {
  ...
  const mapper = ((bytes[7] & 0xf0) | (bytes[6] >> 4)) & 0xff;
  const mirroring = bytes[6] & 0x01 ? "VERT" : "HORZ";
  const hasBattery = (bytes[6] & 0x02) !== 0;
  const hasTrainer = (bytes[6] & 0x04) !== 0;
  const chrStart = 16 + (hasTrainer ? 512 : 0) + prgSize;
...
function drawNESMeta(meta) {
  const lines = [
    `PRG: ${meta.prgBanks}x16K`,
    `CHR: ${meta.chrBanks}x8K`,
    `MAPPER: ${meta.mapper}`,
    `MIRROR: ${meta.mirroring}`,
    `BATTERY: ${meta.hasBattery ? "YES" : "NO"}`,
  ];
  drawText("NES CART INFO", 140, 52, COLOR_PALETTE.orange);
  lines.forEach((line, index) => {
    drawText(line, 140, 70 + index * 16, COLOR_PALETTE.white);
  });
}
